    ...
    FROM       
        (SELECT events.user_id,
                ...
                CASE 
                    ...
                    COUNT(*) AS event_count 
          FROM events
          JOIN users_ab ON events.user_id = users_ab.user_id
                                                ...
                                                (SELECT user_id,
                                                  ...
                                                  FROM
                                                          (SELECT user_id,
                                                                  ...
                                                                  SUM(CASE WHEN event = 'app start' THEN 1 ELSE 0 END) OVER (PARTITION BY user_id ORDER BY user_id, active_date) 
                                                                  AS session_count,
                                                                  time_diff
                                                            FROM
                                                                    (SELECT user_id,
                                                                            ...
                                                                            --calculating time difference and casting time difference to minutes
                                                                            active_time - LAG(active_time) OVER (PARTITION BY user_id ORDER BY user_id, active_date) :: INTERVAL second 
                                                                            AS time_diff 
                                                                      FROM       
                                                                              (SELECT users.user_id,
                                                                                      gender,
                                                                                      ...
                                                                                      CASE 
                                                                                          WHEN event = 'action_1' OR event = 'action_2' THEN 'left or right swipe'
                                                                                          ELSE 'app start'
                                                                                          END AS event
                                                                              FROM users
                                                                              JOIN events
                                                                              ON users.user_id = events.user_id) AS first_query) AS second_query) AS third_query
                                                  WHERE event != 'app start'
                                                  GROUP BY user_id, gender, active_date, active_time, event, event_count, session_count, time_diff
                                                  ORDER BY user_id, active_date, event_count, session_count) AS fourth_query
                                        ORDER BY user_id, active_date, session_count) AS fifth_query
                              GROUP BY user_id, gender, session_count, events_per_session, session_time
                              ORDER BY user_id, session_count) AS sixth_query
                    GROUP BY user_id, gender, events_per_session, session_time) AS seventh_query
          GROUP BY user_id, gender, avg_events_user, avg_minutes_user) AS final_query
GROUP BY gender;          
